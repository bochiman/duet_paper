...
\begin{abstract}
Vision-language-action (VLA) policies fine-tuned on limited demonstrations can struggle to detect and recover from execution failures induced by out-of-distribution (OOD) conditions. Existing solutions often require external detectors or costly recovery data. We introduce DUET, a dual-action-head framework whose action heads are trained only on successful demonstrations. Relative actions provide nominal control, while an absolute head without explicit proprioceptive conditioning predicts RVQ-conditioned waypoints for recovery. Disagreement between the relative command and the waypoint-induced correction, augmented by a training-state bound violation, provides an endogenous failure-risk score that continuously gates the transition between them. On four LIBERO suites under four failure families, DUET improves nominal success from $93.8\%$ to $96.9\%$ and action-execution failure success from $27.9\%$ to $64.6\%$ over its relative-only baseline. Its disagreement score achieves the highest AUROC ($0.845$) among the evaluated success-only detectors. Instantiated on a separately trained $\pi_0$ controller for a Franka arm, DUET preserves nominal success ($90.0\%$) and raises robot-state OOD success from $10.0\%$ to $77.5\%$.
\end{abstract}
...
Runtime monitoring must distinguish unusual observations from deviations that threaten task completion. Sentinel combines temporal action consistency with VLM-based assessment of task progress; we evaluate its action-consistency signal STAC \cite{sentinel}. Without failure examples, FAIL-Detect formulates monitoring as sequential OOD detection using scalar uncertainty signals; we compare against its logpZO, RND, and PCA-kmeans variants \cite{faildetect}. DUET instead compares a relative command with the local correction induced by an absolute waypoint learned from successful demonstrations. Unlike a standalone scalar alarm, the same comparison also exposes a candidate correction for recoverable execution failures.
...
\begin{equation}
    ...
    {u_{\max,j}-u_{\min,j}+\epsilon}
    \right].
\end{equation}
The failure-risk score and recovery weight are $q_t = D_t + \lambda S_t$ and $\alpha_t = \sigma(k(q_t-\tau))$. DUET interpolates local execution and global recovery while keeping relative gripper control:
\begin{align}
    a_t &= [a_{p,t},a_{g,t}], \\
...
\label{sec:detection}
Table~\ref{tab:detection} evaluates whether $D_t$ provides a useful runtime ranking signal, comparing it with success-only detectors under matched protocols \cite{faildetect,sentinel}. Unlike those standalone scores, $D_t$ is coupled to the correction $c_t$; we evaluate it here as a ranking signal rather than a calibrated failure probability.
...
DUET applies a continuous correction weight, not a binary stop command. Across $2{,}000$ clean episodes, $\alpha_t$ has mean $0.139$ and $95$th percentile $0.339$, with only $1.7\%$ of steps above $0.5$. These results answer Q3 narrowly: $D_t$ is a competitive ranking and control signal for the evaluated deviations, not a calibrated failure probability. Q1 evaluates the closed-loop benefit of coupling it to $c_t$.
...
\begin{table}[t]
\caption{Component and gate-score ablations (success, \%). $^\dagger$Projected targets for experiment planning, not measured results.}
\label{tab:ablations}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
...
\midrule
\multicolumn{4}{@{}l}{\textit{Target gate-score decomposition, Object (medium)}} \\
Relative only ($\alpha_t{=}0$) & 99.2 & 36.2 & 48.8 \\
$S_t$ only$^\dagger$ & 99.0 & 52.4 & 56.0 \\
$\lVert c_t\rVert_2$ only$^\dagger$ & 98.8 & 61.6 & 67.2 \\
$D_t$ only$^\dagger$ & 99.0 & 70.8 & 80.6 \\
\rowcolor{gray!20}
...
The target decomposition uses a shared checkpoint and matched Object rollouts. Thresholds are calibrated on disjoint clean rollouts to match nominal activation; other settings are fixed. Theory predicts $D_t$ to dominate action corruption and arm displacement, $S_t$ to respond only to state-bound crossings, and $\lVert c_t\rVert_2$ to conflate recovery with legitimate large motions. The full score should therefore retain clean behavior while outperforming each cue. Daggered entries are planning targets and must be replaced before submission.
...
DUET preserves $90.0\%$ aggregate nominal success (Table~\ref{tab:real_robot}). Robot-state OOD improves from $10.0\%$ to $77.5\%$ across all four tasks, while world-state OOD rises from $6.3\%$ to $58.8\%$. The remaining gap is consistent with changed object relationships being harder to recover than arm alignment, but task-specific interventions preclude severity-matched comparison (Fig.~\ref{fig:real_tasks}). With $20$ trials per cell, the results support these tasks and backbone, not broad cross-robot generalization. No additional detector or recovery dataset is used.

\section{LIMITATIONS}

DUET requires aligned absolute-state labels and hardware calibration. Its min--max proxy can miss multimodal structure, while the absolute branch can lose information not visible in images. $D_t$ is weaker for world-state changes and shared visual corruption; the guard handles only the latter. Fixed-phase injections do not establish robustness to arbitrary timing. Irreversible failures and tasks requiring a new strategy are out of scope.

\section{CONCLUSION}

We presented DUET, a dual-branch VLA for endogenous failure-risk scoring and recovery. Comparing a local relative command with an RVQ-conditioned absolute waypoint yields both an unreliability signal and a closed-loop correction without an external detector or recovery demonstrations. Separately trained StarVLA and $\pi_0$-based controllers demonstrate applicability across the two evaluated backbones rather than direct weight transfer.
