...
Given images $o_t$, instruction $l$, and proprioceptive state $s_t$, DUET predicts relative action and absolute waypoint chunks over a horizon $K$:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Commands decompose as $a_t=[a_{p,t},a_{g,t}]$, with six pose dimensions $a_{p,t}$ and one gripper dimension $a_{g,t}$. Relative outputs are operational-space-control deltas; absolute outputs encode future end-effector poses and gripper states. Only pose commands are fused; deployed gripper control comes from the relative head.
...
We model successful end-effector trajectories as a manifold $\mathcal{M}_l\subset SE(3)$. Near this support, relative commands describe local tangent motion in $T_{p_t}\mathcal{M}_l$, while absolute waypoints reference global coordinates on $\mathcal{M}_l$. After a deviation, a relative command continues from the displaced pose, whereas an absolute waypoint supplies a persistent target toward demonstrated behavior. Their transported predictions should align in-domain; OOD execution can expose inconsistency between local continuation and the global reference. This geometric interpretation motivates detection without guaranteeing it.

Both heads share a VLM encoder (Fig.~\ref{fig:architecture}). The relative head attends to its full hidden sequence and receives $s_t$ through a state encoder. The absolute head excludes explicit proprioception and state tokens from conditioning, preserving an observation-conditioned reference. State tokens, when present, follow visual-language tokens in the causal encoder and are omitted from absolute-branch pooling.

\subsection{Disagreement-Preserving Learning}
Successful demonstrations supply paired delta actions and normalized future states; no failure labels or human takeover actions are required. We optimize a weighted sum of the two flow-matching losses \cite{pi0} and an RVQ commitment loss, without cross-head consistency supervision that could suppress detection evidence. RVQ quantizes pooled state-free visual-language features into a discrete condition atlas learned from demonstrations, anchoring absolute predictions \cite{vqvae,rvq}; its residual is retained only as auxiliary telemetry.

Asymmetric augmentation trains the absolute head and RVQ on clean and perturbed observations, but computes relative-head loss only on clean samples, encouraging a robust reference and deviation-sensitive local control. For classifier-free guidance (CFG) \cite{cfg}, condition dropout replaces absolute-head conditions with learned null tokens to learn conditional and marginal waypoint flows.
...
\end{equation}
Here $D_t$ supplies scalar detection evidence, while $c_t$ supplies a six-dimensional retry direction. The auxiliary $S_t$ averages coordinatewise distances outside the training pose bounds, normalized by each coordinate's training range. The control score $q_t$ and fusion weight are
\begin{equation}
\begin{aligned}
    q_t &= D_t + \lambda S_t, \\
...
\end{aligned}
\end{equation}
Here $\sigma$ is the sigmoid, $k$ its slope, and $\tau$ the switching threshold; $\lambda$ weights state distance. The executed command is
\begin{equation}
\begin{aligned}
...
\end{equation}
CFG strength follows $w_t=w_{\max}-(w_{\max}-w_{\min})\alpha_{t-1}$ for the next chunk. When the original trajectory remains feasible, rising disagreement shifts control toward $c_t$ for retry, and realignment restores relative execution.
...
\end{itemize}
We evaluate these questions with matched LIBERO experiments, controlled execution disturbances, detector-score analysis, component ablations, and a real-robot protocol. Detector quality, nominal task success, and closed-loop retry success are reported separately.
