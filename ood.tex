...
DUET exploits complementary action representations. Relative actions support smooth local control but provide no return target after displacement; absolute actions are less suitable for fine control but retain a global waypoint. DUET learns the same behavior with a relative head for execution and an absolute head for trajectory re-entry. Comparing the transported waypoint correction with the relative action tests whether local motion remains consistent with the demonstrated reference. Their disagreement is a task-specific inconsistency signal, not a universal OOD score, and the waypoint supplies a retry direction.
