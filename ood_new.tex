...
Achieving this poses two coupled challenges. Successful demonstrations specify how to act along supported trajectories, but provide neither explicit failure labels nor corrective actions after departure. DUET addresses both through a structural asymmetry between action representations \cite{aljalbout2024actionspace,feng2026demystifying}, rather than a separately supervised failure model. State-conditioned relative commands provide precise local control but no explicit return target, whereas state-excluded absolute predictions retain a scene-conditioned waypoint. Neither representation is assumed universally superior; DUET assigns them complementary roles. Crucially, we compare the predictions only after mapping them into the same normalized command space, preventing their different parameterizations from masquerading as behavioral disagreement. Along supported execution, the waypoint correction and relative command should broadly agree. After a robot-state deviation, the waypoint can remain stable while its required correction changes, exposing a cross-space mismatch. Their disagreement then provides an endogenous risk signal, while the waypoint supplies an actionable return direction.

Both heads learn paired continuations from the same successful demonstrations and require no failure or recovery labels. Failure awareness therefore emerges from paired success supervision and asymmetric conditioning rather than labeled negative trajectories. The live pose converts the absolute waypoint into local command units but is not an explicit input to its predictor. Because corrupted observations can degrade both heads together, disagreement is treated as a heuristic risk signal and validated empirically rather than assumed to detect every failure.

The module attaches to shared vision-language features and is instantiated with StarVLA \cite{starvla} in simulation and $\pi_0$ (OpenPi) \cite{pi0} on hardware. At inference, a soft gate shifts toward waypoint correction as disagreement rises and restores relative control after realignment. Irreversible failures, hardware faults, and cases requiring a new task strategy remain outside its scope.

Our contributions are threefold:
\begin{itemize}
    \item We formulate cross-space action disagreement as an endogenous failure-risk signal learned without failure labels; it outperforms the evaluated success-only detectors on TPR and AUROC (\S\ref{sec:detection}).
    \item We propose a closed-loop retry controller that uses the same absolute prediction for waypoint correction and continuously hands control back to the relative head after realignment (\S\ref{sec:nominal}, \S\ref{sec:retry}).
    \item We evaluate the approach with two VLA backbones: StarVLA in LIBERO and a separately trained $\pi_0$ (OpenPi) controller across four Franka tasks (\S\ref{sec:retry}, \S\ref{sec:real_robot}).
\end{itemize}
...
Runtime monitors estimate risk from policy confidence, temporal inconsistency, or semantic progress. FAIL-Detect performs sequential OOD detection without failure examples using policy likelihoods or success-only features; we compare its logpZO, RND, and PCA-kmeans variants \cite{faildetect}. Sentinel combines action consistency with VLM-based progress assessment; we evaluate its training-free STAC signal \cite{sentinel}. Such monitors map a score to an alarm, whose threshold trades missed failures against false interventions, but neither establishes recoverability nor specifies a corrective action. We therefore evaluate detection ranking separately from closed-loop retry.

DUET is closest to action-consistency monitoring, but its comparison is deliberately asymmetric: a state-conditioned relative command is contrasted with the correction induced by a state-excluded waypoint. The residual is a heuristic ranking, while the same pair provides both the warning and recovery direction, coupling monitoring to control without an external detector.

\subsection{Recovery from Execution Failures}
One route expands the training distribution. DAgger queries experts on policy-induced states \cite{dagger}; intervention learning obtains corrective actions from human takeovers \cite{eil,sirius}; and IntervenGen synthesizes trajectories from a small intervention set \cite{intervengen}. RACER combines recovery demonstrations, language, and VLM supervision \cite{racer}, while optimized VLM prompts support motion- or task-level correction \cite{vlmrecovery}. These approaches can learn recovery beyond the demonstrated strategy, but require additional intervention, supervision, or inference machinery.

Recovery can instead reuse successful experience. Back to the Manifold returns toward high-density training states \cite{backtomanifold}, and OCR learns an inverse recovery policy from object-keypoint structure \cite{ocr}. Rewind-IL respawns from VLM-verified demonstration checkpoints \cite{rewindil}; FAR adapts at test time using failure-derived preferences \cite{far}. Thus, avoiding recovery demonstrations is not unique to DUET. Its distinction is a single closed loop in which jointly trained action views supply the trigger, return direction, and handoff to nominal control, without checkpoint retrieval or test-time updates. This limits DUET to recoverable deviations but avoids requiring a separate recovery policy.

DUET adopts the narrower assumption that successful experience contains a useful return target, but generates it from the current observation rather than retrieving a stored state; re-evaluating both heads keeps recovery and handoff closed loop.

\subsection{Action Representations for Robust Control}
Action-space design changes the invariances, optimization, and transfer behavior of robot policies \cite{aljalbout2024actionspace,feng2026demystifying}. Relative commands express local motion and often transfer across workspace locations, whereas absolute targets retain a global reference but couple prediction to coordinates and calibration. Prior studies largely compare these representations as alternatives for nominal prediction. DUET does not introduce a new action space; it retains both views and assigns them complementary roles. State-conditioned relative commands execute locally, while state-excluded absolute waypoints define return targets. Converting both to common command units makes their disagreement operational: it identifies an execution mismatch, and the absolute view remains directly usable for correction.

The outputs are not interchangeable ensemble samples: they share visual-language features and paired successful labels, but differ deliberately in action frame and state conditioning. This structured asymmetry makes their residual informative after robot-state displacement, without failure labels for arbitration.

\section{METHOD}

\subsection{Problem Formulation}
At time $t$, the robot observes multi-view images $o_t$, a language instruction $l$, and proprioceptive state $s_t$. Trained only on successful demonstrations, DUET predicts the same continuation in two action representations over a horizon $K$:
\begin{align}
    ...
    \in \mathbb{R}^{K\times 7}.
\end{align}
$\Delta A_t$ contains relative operational-space commands and $W_t$ absolute waypoints $[x,y,z,r_x,r_y,r_z,g]$. Recorded deltas and future end-effector states label the same successful continuation; no failure or recovery action is used. Inference caches both length-$K$ chunks. At each control step, $\hat{\delta a}_{p,t}$ and $\hat{w}_{p,t}$ denote their current 6-D pose elements. Fusion acts only on pose; gripper control remains relative.
...
For instruction $l$, let $\mathcal{S}_l$ denote the execution region represented by successful demonstrations; DUET uses this support only as an interpretation and does not estimate a geometric manifold. The relative head uses the live state to answer \emph{how to move next}, providing precise local control but no explicit return destination. The absolute head excludes explicit numerical proprioception and instead predicts \emph{where to be}, yielding a coarser waypoint anchored by visual context and successful behavior.

Raw deltas and absolute coordinates are not directly comparable. We therefore convert $\hat{w}_{p,t}$ at the live pose into a normalized one-step correction $c_t$ (Eq.~\eqref{eq:recovery_vector}). On supported execution, $c_t$ and $\hat{\delta a}_{p,t}$ describe the same continuation and should broadly agree. After a state displacement, the waypoint can remain stable while its required correction changes, exposing divergence from the local command. We use this cross-space residual as a heuristic risk score and $c_t$ as the associated return direction. Agreement is neither necessary nor sufficient for success---both heads may fail together under corrupted observations---so Sec.~\ref{sec:detection} evaluates the signal empirically.

\subsection{Dual-Head Architecture}
Figure~\ref{fig:architecture} shows a shared VLM and two flow-matching heads \cite{diffusionpolicy,pi0}. The relative head receives $s_t$ through a numerical state encoder and a terminal $\langle\mathrm{state}\rangle$ prompt segment. The absolute head has no state encoder; its pooling mask excludes padding, that segment, and all later tokens that may carry state information. This blocks the direct numerical shortcut $W_t\!\approx\!s_t+\Delta$, although pose remains partially visible in the images. For shared hidden sequence $H$, residual vector quantization (RVQ) \cite{vqvae,rvq} forms

\begin{equation}
    Z_q=\operatorname{RVQ}\!\left(\operatorname{Pool}(H;m_{\mathrm{abs}})\right),
\end{equation}
where $m_{\mathrm{abs}}$ is this exclusion mask. $Z_q$ is a conditioning bottleneck, not a retrieved trajectory; the absolute flow head still generates $W_t$. Its quantization residual is telemetry only. Classifier-free guidance (CFG) \cite{cfg} controls conditional strength at inference.
...
\end{equation}
where absolute labels are the next $K$ normalized end-effector states, relative labels are recorded deltas, and all three outer weights are $1$. Paired supervision already encourages agreement on successful behavior. We add no direct consistency loss because, especially on augmented inputs, it could suppress the residual used for monitoring.

Asymmetric visual augmentation specializes the branches. With probability $p_{\mathrm{aug}}$, each view independently receives one or two of color jitter, an $80$--$95\%$ resized crop, and Gaussian pixel noise ($\sigma{=}5$--$25$ on $[0,255]$), without changing targets. Absolute and RVQ losses use the full batch; relative loss uses only clean samples, preserving a sensitive local head and robust reference. For CFG, the absolute branch replaces $Z_q$ by learned null tokens on random samples.

\subsection{Joint Failure-Risk Scoring and Recovery}
\label{sec:method-gate}
The two heads are queried only when a new chunk is needed. At every environment step, DUET retrieves the current cached elements and recomputes the correction from the live pose $p_t=(x_t,R_t)$ and waypoint $\hat{w}_{p,t}=(\hat{x}_t,\hat{R}_t)$:
\begin{equation}
...
\end{equation}
where $\operatorname{Log}^{\vee}$ returns the controller's $SO(3)$ rotation vector. We use $\sigma_p{=}0.05$\,m and $\sigma_R{=}0.5$\,rad, matching its unit-command scales. This geodesic form avoids the spurious correction caused by element-wise axis--angle subtraction near a representation flip. Cross-space disagreement is
\begin{equation}
    D_t=\frac{\left\|c_t-
    \operatorname{clip}(\hat{\delta a}_{p,t},-1,1)\right\|_2}{\sqrt{6}}.
    \label{eq:disagreement}
\end{equation}
$D_t$ is the RMS discrepancy over six clipped pose channels, hence $D_t\in[0,2]$; it is a command-space scale, not a failure probability. A secondary term measures training-bound violation. Let $u_t\in\mathbb{R}^7$ be the measured state in waypoint coordinates (translation, canonicalized rotation vector, and gripper), $u_{\min},u_{\max}$ its coordinatewise training bounds, and $[z]_+=\max(z,0)$. Then
\begin{equation}
    S_t =
    ...
    \right].
\end{equation}
We use $\epsilon{=}10^{-6}$. In-bound coordinates contribute zero, so $S_t$ measures box violation, not in-box density. The score and recovery weight are $q_t=D_t+\lambda S_t$ and $\alpha_t=\sigma(k(q_t-\tau))$, where $\tau$ is the equal-mixture crossover and $k$ the transition slope. We fix $\lambda{=}1$, $\tau{=}0.5$, and $k{=}8$ across all suites and protocols without fitting evaluation rollouts. Thus $\alpha(0/0.5/1){=}0.018/0.5/0.982$: agreement favors relative control, whereas a unit risk score rapidly activates correction without a hard switch. DUET fuses pose while retaining relative gripper control:
\begin{align}
    a_t &= [a_{p,t},a_{g,t}], \\
...
\end{align}
The absolute gripper output is a state label, not a command, and is discarded. Rising $q_t$ shifts pose control toward $c_t$; after realignment it returns to the relative head. At chunk refresh, the latest fusion weight from a valid observation sets $w=w_{\max}-(w_{\max}-w_{\min})\alpha_{\mathrm{valid}}$, held for that chunk. Thus $q_t$ determines \emph{when} and $c_t$ \emph{how} to recover. Shared visual corruption can invalidate both heads and their disagreement. The guard marks either normalized RGB view invalid if pixel standard deviation is below $0.02$ or mean intensity below $0.01$. It then skips inference, consumes the last healthy absolute chunk, and holds its final waypoint with $\alpha_t{=}1$; this override does not update $\alpha_{\mathrm{valid}}$.
...
\emph{Grasp and vision.} A $5$\,cm drop yields a significant $6.0$ pp gain from an $89.2\%$ baseline ($p{=}2.4{\times}10^{-5}$; coverage $\geq98.8\%$). Restoring arm pose need not restore a changed grasp. During blackout, the validity guard consumes the last healthy absolute chunk and then holds its final waypoint; its aggregate gain is therefore evidence for the full robustness system rather than disagreement-based failure detection alone.
...
\label{sec:ablations}
Table~\ref{tab:main_results} separates adaptive switching from adding an absolute prediction. Absolute-only and fixed fusion reduce Object clean success to $5.2\%$ and $46.2\%$; waypoint guidance is not a complete policy. Adaptive fusion matches relative-only clean success and gains $39.8/35.6$ pp under displacement/action perturbation. Across four suites, it gains $3.1/21.7/36.7$ pp on clean/displacement/action execution. The displacement row uses medium severity; Table~\ref{tab:ood_detailed} pools both severities.
...
Table~\ref{tab:ablations} separates inference guidance from training choices. On Object, removing adaptive CFG loses $11.0/23.4/27.2$ pp on clean/displacement/action execution, showing that chunk-level conditioning contributes beyond pose interpolation. On LIBERO-10, symmetric augmentation loses $5.0/2.2/2.9$ pp, supporting branch specialization. The RVQ-to-MLP replacement collapses in this configuration; this diagnoses that replacement, but does not by itself establish that discrete quantization is universally superior to a matched continuous bottleneck. Comparisons remain within suite-specific blocks.
...
Alternative-score thresholds use disjoint clean Object data to match nominal activation, distinct from controller $\tau$. The correction norm improves OOD success but also responds to legitimate large motions; $S_t$ acts only after a coordinate crosses its training bound. $D_t$ discounts large but mutually consistent commands and is the strongest individual signal. Adding $S_t$ to $D_t$ contributes another $5.2/3.8$ pp under displacement/action perturbation at matched clean success. Together with the constant-gate baselines, these results attribute the gain to adaptive, disagreement-led fusion without treating $(\tau,k)$ as optimized.

\subsection{Real-Robot Evaluation}
\label{sec:real_robot}
We test the same principle on a separately trained $\pi_0$ (OpenPi) controller \cite{pi0}, without transferring simulation weights. A 7-DoF Franka uses wrist and third-person cameras at $20$\,Hz; four tasks spanning grasping, contact, and articulated manipulation are each fine-tuned for $5$k steps from $\pi_0$-base on $50$ VR demonstrations (Fig.~\ref{fig:real_tasks}, left).

Each task uses $N{=}20$ trials per cell under nominal execution, paired random-direction EEF shifts, and object displacements. Task-specific interventions alter active manipulation; both methods share each specification.
...
DUET preserves $90.0\%$ nominal success (Table~\ref{tab:real_robot}). Robot-state OOD rises from $10.0\%$ to $77.5\%$ (Wilson $95\%$ CIs: $5.2$--$18.5\%$, $67.2$--$85.3\%$), gaining at least $65$ pp per task. Despite possible waypoint invalidation, world-state OOD rises from $6.3\%$ to $58.8\%$ ($2.7$--$13.8\%$, $47.8$--$68.9\%$). One robot and $20$-trial cells nevertheless limit generality.

\section{DISCUSSION}

DUET turns representation mismatch into a warning and correction direction. Controlled comparisons attribute gains to online fusion rather than co-training or averaging. Geometric re-entry alone is insufficient because absolute-only cannot resume local behavior; adaptive fusion returns control to the relative head after realignment. DUET is a retry controller, not a semantic replanner.

\section{LIMITATIONS}

DUET requires aligned absolute-state labels and controller-scale calibration. Coordinatewise bounds ignore multimodality, and the state-excluded head can miss hidden information. Correlated errors can yield false agreement and legitimate actions false alarms; changed object relations may invalidate waypoints. Fixed-phase, single-seed simulation and one robot with $20$-trial cells limit generality. Contact repair, object search, and new strategies remain out of scope.

\section{CONCLUSION}

We presented DUET, a dual-action VLA that couples risk scoring and recovery using successful demonstrations alone. Mapping an absolute waypoint into live command space makes it comparable with relative action: disagreement raises recovery weight, while the same correction guides the robot until relative control resumes. Across LIBERO and a separately trained $\pi_0$ Franka controller, DUET improves recoverable OOD execution while preserving nominal performance, without an external detector or recovery data. DUET remains a retry mechanism rather than a semantic replanner; correlated prediction failures and invalid return waypoints require richer recovery models.
